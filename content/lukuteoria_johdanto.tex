...
\begin{figure}[h!]
	...
	\caption{Saksalainen pankkiautomaatti. Wikimedia Commons: Stahlkocher / GFDL 1.2}
	% https://commons.wikimedia.org/wiki/File:Fr%C3%BCher_Bankautomat_von_Nixdorf.jpg
\end{figure}
...
\input{content/TEHT5-1} % TEHTÄVÄT
...
\numerointi{
§ Ryhmittele luvut $16$, $59$, $12$, $3$, $177$, $21$, $47$, $65$, $222$, $-17$, $-100$ ja $-1$ niin, että samaan joukkoon kuuluvat ne luvut, joilla on sama jakojäännös, kun ne jaetaan luvulla $5$.
§ Valitse ensin kaksi lukua, jotka ovat samassa kohdan 1 joukossa. Laske niiden erotus. Toista kaksi kertaa. Valitse sitten kaksi lukua, jotka ovat eri joukoissa kohdassa 1. Laske niiden erotus. Toista kaksi kertaa. Miten erotuksen arvosta näkee, kuuluvatko vähenevä ja vähentäjä samaan kohdan 1 joukkoon?
§ Miten voit esittää yleisessä muodossa kaikki ne kokonaisluvut, jotka ovat jaollisia luvulla $5$? Entä ne luvut, joiden jakojäännös on $1, 2, 3$ tai $4$, kun jaetaan luvulla $5$?
§ Todista kohdan 2 havaintosi.
}
...
%{\bf Esimerkki 3.} 
\alakohdat{
§ Määritä pienin epänegatiivinen kokonaisluku, joka on kongruentti luvun $45$ kanssa modulo $7$.
§ Jos tänään on perjantai ja loman alkuun on $45$ päivää, niin minä viikonpäivänä loma alkaa?
}
...
\numerointi{
§ $a+b\equiv c+d\quad (\mod k)$,
§ $a-b\equiv c-d\quad (\mod k)$,
§ $ab \equiv cd\quad (\mod k)$,
§ $a^n \equiv c^n \quad (\mod k)$.
}
}
...
\input{content/TEHT5-2} % TEHTÄVÄT
...
\alakohdat{
§ luku $7^{650}$ jaetaan luvulla $6$
§ luku $7^{650} - 194$ jaetaan luvulla $8$
§ luku $0+1+2+3+ \ldots + 68 + 69$ jaetaan luvulla $7$.
}
...
\input{content/TEHT5-3} % TEHTÄVÄT
...
\numerointi{
§ Luvut $0$ ja $1$ ovat luonnollisia lukuja.
§ Jos $n$ on luonnollinen luku, niin luku $n+1$ on myös luonnollinen luku.
}
...
\numerointi{
§ Osoitetaan, että tulos pätee, kun $n=0$. Tämä vaihe on yleensä yksinkertainen lasku. Induktio voidaan aloittaa myös esimerkiksi luvusta $n=1$.
§ Kiinnitetään $k\in \nn$. Oletetaan, että tulos on tosi luvulla $n=k$. Osoitetaan, että tulos on tosi luvulla $n=k+1$.
§ Nyt induktioperiaatteesta seuraa, että tulos on tosi kaikilla $n\in \nn$.
}
...
\input{content/TEHT5-4} % TEHTÄVÄT
